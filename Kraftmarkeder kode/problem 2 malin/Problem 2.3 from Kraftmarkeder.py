...
Problem 2.3: DCOPF with multiple generators at Node 1

Extends the DCOPF from Task 2-2a by replacing the single generator at
Node 1 with three generators of different costs and capacities.
All other system parameters (loads, lines) remain unchanged.

Data: "Problem 2 data.xlsx", sheet "Problem 2.3 - Generators".
...
df = pd.read_excel(file_path, sheet_name="Problem 2.3 - Generators", header=None)

# Generators: rows 3-7, cols 0-3 (5 generators: Gen1_1, Gen1_2, Gen1_3, Gen2, Gen3)
n_gens = 5
...
# Loads: rows 3-5, cols 9-11 (same as 2.2: D1=200, D2=200, D3=500)
load_demands = df.iloc[3:6, 10].astype(float).tolist()
load_nodes   = [int(str(s).split()[-1]) for s in df.iloc[3:6, 11]]
demand = dict(zip(load_nodes, load_demands))

# Lines: rows 3-5, cols 15-17 (same as 2.2)
line_cap_list  = df.iloc[3:6, 16].astype(float).tolist()
line_susc_list = df.iloc[3:6, 17].astype(float).tolist()  # negative
line_keys = []
for s in df.iloc[3:6, 15]:
...
model = pyo.ConcreteModel("DCOPF_Task2_3")
...
# Power balance — key change from 2-2: multiple generators at same node
...
print("=" * 60)
print("  Task 2-3: DCOPF — Multiple Generators at Node 1")
print("=" * 60)
...
for g in g_names:
    ...
    print(f"  {g:8s} (Node {generators[g]['node']}, "
          ...
          f"{pg:8.2f} MW  (cap: {cap:.0f})  [{status}]")
...
# ==========================================================
# 4. Plot: stacked bar chart of generation per node
# ==========================================================
...
for i, n in enumerate(nodes):
    ...
    y_pos = max(total_gen, demand[n]) + 15
...
ax.set_title('Task 2-3: Generation Dispatch per Generator\n'
             '(dashed line = nodal demand)', fontsize=12)
ax.legend(handles=patches, loc='upper right', fontsize=8)
ax.set_ylim(0, max(bottoms) * 1.25)
ax.grid(axis='y', alpha=0.3)
plt.tight_layout()
plt.savefig('task_2_3_generation.png', dpi=150, bbox_inches='tight')
plt.show()
